...
from metrics.cooc import pmi, bias_logoddsratio
...
logoddsratio = bias_logoddsratio(
...
#%% print results
with open(f'results/dpmi_{TARGET_A}-{TARGET_B}-{CONTEXT}.md', "w") as f:
    print(
        f'with {VOCAB_FILE} :\n'
        ,'\n### PMI and Odds Ratio \n'
        ...
        ,f'- logOddsRatio = {logoddsratio[0]}\n'
        ,f'- CI-logOddsRatio = {odds_ratio[1]} -- {odds_ratio[2]}\n'
        # ,'\n### Word counts \n'
        # ,f'- {TARGET_A}: {count_a}\n'
        # ,f'- {TARGET_B}: {count_b}\n'

...
def pmi(cooc_dict, words_target, words_context, str2count, alpha=.75):
    ...
    count_context_alpha = sum([str2count.get(word,0)**alpha for word in words_context])
...
def bias_ppmi_bydoc(words_target_a, words_target_b, words_context
    ...
    """Return pd.DataFrame with one row by doc and columns:
        ...
        - line of corpus
        - name
        - odds ratio a/b con target_a, target_b, context
    TODO: hallar expresion para aporte de cada doc a Odds Ratio
        Odds Ratio de cada doc no funciona porque en muy pocos docs hay
        target_a target_b y context en simultaneo
        (en general solo hablan de un target)
    """
    ...
    # for each doc: read metadata + read doc + cooc_matrix + odds_ratio
    result = {'id':[], 'line':[], 'name':[], 'diff_ppmi':[]} # init results dict
    ...
    with open(corpus, "r", encoding="utf-8") as f:
        while True:
            ...
            cooc_dict = create_cooc_dict(doc, word_list, window_size=window_size)
            ...
            if not words_target_cooc:
                continue
            pmi_a = 0 # pmi default (TODO: en realidad nunca puede ser 0)
            pmi_b = 0
